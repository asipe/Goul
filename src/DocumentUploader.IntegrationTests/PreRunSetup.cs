...
namespace DocumentUploader.IntegrationTests {
  ...
  public sealed class GlobalSetup {
    ...
    public void DoSetup() {
      ...
      if (!File.Exists("Credentials_Test_Use_Only.txt") || File.ReadAllLines("Credentials_Test_Use_Only.txt").Length < 2) {
        Assert.Fail("Credentials file for test use could not be found or it does not have the correct amount of credentials");
      }
